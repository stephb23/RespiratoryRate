...
% Read in the file
fileID = fopen('albusRRPeriodicBiLSTM60s.txt', 'r');
valueArray = textscan(fileID, 'y_pred = [%f], y_true = [%f]');
calculatedRRs = (valueArray{1});
expectedRRs = valueArray{2};

% Calculate key parameters
meanAbsoluteError = mean(abs(expectedRRs - calculatedRRs));
stdDev = std(expectedRRs - calculatedRRs)
meanError = mean(expectedRRs-calculatedRRs);
correlation = corrcoef(expectedRRs, calculatedRRs)

length(calculatedRRs)

% Calculate features of Bland Altman plot
meansOfRRs = (expectedRRs + calculatedRRs)/2;
differenceOfRRs = expectedRRs - calculatedRRs;
standardDeviation = std(differenceOfRRs);
bias = mean(differenceOfRRs);
lowerLOA = bias - 1.96*standardDeviation;
upperLOA = bias + 1.96* standardDeviation;

loaDiff = upperLOA - lowerLOA

counter = 0;
for i = 1:length(meansOfRRs)
    if abs(15 - meansOfRRs(i)) <= 0.1 & abs(differenceOfRRs(i)) <= 0.1
        counter = counter + 1;
    end
end
counter 
...
counter2 = 0;
mode(calculatedRRs)
for i = 1:length(calculatedRRs)
    if expectedRRs(i) == 15 & calculatedRRs(i) == 14.9320
        counter2 = counter2 + 1;
    end
end
counter2

% Calculate how many differences fall in the main region
aboveLowerLOA = differenceOfRRs(differenceOfRRs > lowerLOA);
insideLOAs = sum(aboveLowerLOA < upperLOA);
outsideLOAs = length(differenceOfRRs) - insideLOAs;
bias
loaDiff = upperLOA - lowerLOA
percentageInsideLOAs = insideLOAs/length(differenceOfRRs) * 100
fprintf('%.2f & %.2f & %.2f\n', bias, loaDiff, percentageInsideLOAs);

% Construct Bland Altman plot
figure('units','normalized','outerposition',[0 0 1 1]);
hold on;
% axis([0, 35, -12, 12])
dscatter(meansOfRRs, differenceOfRRs);
ylim([-25, 20])
xlim([5, 35])
plot(get(gca,'xlim'), [upperLOA upperLOA], '--k', 'LineWidth', 2);
plot(get(gca,'xlim'), [bias bias], 'k', 'LineWidth', 2);
plot(get(gca,'xlim'), [lowerLOA lowerLOA], '--k', 'LineWidth', 2);
% title('Bland-Altman Plot', 'FontSize', 25);
ylabel('Difference', 'FontSize', 25);
xlabel('Mean', 'FontSize', 25);
legend({'Data', 'Limits of Agreement', 'Mean Difference'}, 'FontSize', 25);
colorbar('FontSize', 25);
title('Bland Altman Plot (60-second PPG & ECG segments)', 'FontSize', 30)
